...
export default function RightPanel() {
  ...
  return (
    <div className="bg-white rounded-[24px] p-6 shadow-card border border-border-subtle w-full max-w-[380px]">
      <h3 className="text-2xl font-extrabold text-text-primary leading-tight mb-8">
      ...
      <div className="flex justify-between mb-10">
        {days.map((d) => (
          <div key={d.date} className="flex flex-col items-center">
            <span className={`text-[18px] font-bold mb-1 ${d.active ? 'text-white' : 'text-text-primary'}`}>
              {d.active ? (
            ...
            <span className={`text-[11px] font-bold uppercase ${d.active ? 'text-primary' : 'text-text-muted'}`}>
              {d.day}
            </span>
          </div>
        ))}
          ...
          todayAppointments.slice(0, 3).map((apt, index) => (
            <div key={apt.id} className="flex items-center gap-4">
              <div className="w-[52px] text-right">
                <p className="text-[12px] font-bold text-text-muted leading-tight">
              ...
              >
                <div className="absolute left-[-4px] w-[48px] h-[48px] rounded-full border-[3px] border-white overflow-hidden shadow-md">
                  <img src={`https://picsum.photos/seed/${apt.patientName}/48/48`} alt="Patient" referrerPolicy="no-referrer" />
                </div>
                <div className="flex flex-col">
                  <span className="text-[15px] font-bold text-primary leading-none mb-1 truncate max-w-[180px]">{apt.patientName}</span>
                  <span className="text-[13px] font-bold text-primary/60 leading-none">{apt.startTime} - {apt.endTime}</span>
                </div>
              </div>
            </div>
          ))
