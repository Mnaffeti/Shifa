...
function BarTooltip({ active, payload }: any) {
  ...
  return (
    <div className="bg-primary text-white px-3 py-1.5 rounded-xl shadow-lg text-sm font-bold">
      {payload[0].value} RDV
    </div>
  );
}
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h3 className="text-xl font-bold text-text-primary">Activité de la semaine</h3>
            <p className="text-sm text-text-muted font-medium mt-0.5 tabular">
              {weekTotal} rendez-vous cette semaine
            </p>
          </div>
          <div className="flex items-center gap-4 text-xs font-bold text-text-muted">
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-[#C8E04A] inline-block" />
              Aujourd'hui
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-[#B2EAE8] inline-block" />
              Passé
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-[#E8F4F4] inline-block" />
              À venir
            </span>
          </div>
        </div>
            ...
            <BarChart data={weeklyData} margin={{ top: 4, right: 0, left: -28, bottom: 0 }}>
              ...
              <Bar dataKey="total" radius={[6, 6, 0, 0]} barSize={34}>
                {weeklyData.map((d, i) => (
                  <Cell
                    key={i}
                    fill={d.isToday ? '#C8E04A' : d.isPast ? '#B2EAE8' : '#E8F4F4'}
                  />
                ))}
              </Bar>
            </BarChart>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="card flex flex-col">
          <h3 className="text-xl font-bold text-text-primary mb-5">Types de consultations</h3>
          {typeData.length > 0 ? (
              ...
              <div className="flex flex-col gap-3 flex-1">
                {typeData.map(d => (
                  <div key={d.name} className="flex items-center gap-2.5">
                    ...
                    <span className="text-sm font-bold text-text-primary tabular">{d.value}</span>
                  </div>
                ))}
                <div className="flex items-center gap-2.5 border-t border-border-subtle pt-3 mt-1">
                  ...
                  <span className="text-sm font-bold text-text-secondary flex-1">Total</span>
                  <span className="text-sm font-bold text-text-primary tabular">
        ...
        {/* Today summary */}
        <div className="card flex flex-col">
          <h3 className="text-xl font-bold text-text-primary mb-5">Aujourd'hui</h3>
          <div className="flex flex-col gap-3 flex-1 justify-center">
            <div className="flex items-center justify-between p-3 bg-bg-soft rounded-xl">
              ...
              <span className="text-2xl font-bold text-text-primary tabular">{todayApts.length}</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-bg-soft rounded-xl">
              <div className="flex items-center gap-3">
              ...
              <span className="text-2xl font-bold text-text-primary tabular">{completed}</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-bg-soft rounded-xl">
              <div className="flex items-center gap-3">
              ...
              <span className="text-2xl font-bold text-text-primary tabular">{upcoming}</span>
            </div>
          </div>
        </div>
